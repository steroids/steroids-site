/* eslint-disable no-multi-str */
export const githubLink = 'https://github.com/steroids/react';
export const featureCards = [
    {
        title: 'Более 70 компонентов',
        description: 'Создавайте сложные интерфейсы \
        в библиотеке с более 70 различных UI компонентов, \
        с настраиваемым цветом, размером и другими характеристиками',
        img: 'https://i.ibb.co/rfkrhr9/1.jpg',

    },
    {
        title: 'Тёмная тема',
        description: 'И как же без darkmode в современных интерфейсах',
        img: 'https://i.ibb.co/2SLXsnh/image.jpg',
        isDark: true,
    },
];

export const advantagesCards = [
    {
        img: '',
        description: 'Открытый код и документация, библиотека компонентов, открытый код, React, Vue',
    },
    {
        img: '',
        description: 'Открытый код и документация, библиотека компонентов, открытый код, React, Vue',
    },
    {
        img: '',
        description: 'Открытый код и документация, библиотека компонентов, открытый код, React, Vue',
    },
    {
        img: '',
        description: 'Открытый код и документация, библиотека компонентов, открытый код, React, Vue',
    },
];

export const advantagesDescription = __('Наш Open-Source \
фреймворк Steroids \
это экосистема на основе \
React и Redux с большим набором готовых \
компонентов, уникальной архитектурой, UI Kit и SSR');

export const resourcesCards = [
    {
        img: 'https://i.ibb.co/6JQcWrX/Frame-338670.jpg',
        title: 'На старт',
        description: 'Открытый код и документация, библиотека компонентов, открытый код, React, Vue',
    },
    {
        img: 'https://i.ibb.co/YjFYxBr/Frame-338670-1.jpg',
        title: 'Figma Design Kit',
        description: 'Набор инструментов для создания интерфейсов для дизайнера',
    },
    {
        img: 'https://i.ibb.co/PDtcKXy/Frame-338670-2.jpg',
        title: 'GitHub',
        description: 'Открытый код и документация, React, Vue',
    },
];

export const resourcesDescription = __('Наш Open-Source фреймворк Steroids \
это экосистема наоснове React \
иRedux сбольшим набором готовых компонентов, \
уникальной архитектурой, UIKit иSSR');
export const examplesCards = [
    {
        label: 'Датапикер',
        img: '/images/calendar.webp',
    },
    {
        label: 'Карточка',
        img: '/images/card.webp',
    },
    {
        label: 'Пароль',
        img: '/images/password.webp',
    },
    {
        label: '',
        img: '',
        isMoreCard: true,
    },
];

export const heroDescription = __('Наш Open-Source фреймворк Steroids \
                это экосистема на основе React и Redux \
                с большим набором готовых компонентов, \
                уникальной архитектурой, UI Kit и SSR.');

export const footerCredits = '© Steroids 2023';
